...
from libc.math cimport exp, abs, pow, sqrt, floor
...
def getk3(s, gs, dur, L=1, N=1000):
    ...
    D = np.array([
        [1 - s, 0, 0, 0, 0],
        [0, np.power(1-s, 2), 0, 0, 0],
        [0, 0, np.power(1-s, 2), 0, 0],
        [0, 0, 0, np.power(1-s, 2), 0],
        [0, 0, 0, 0, np.power(1-s, 3)]
    ])

    v0 = np.array(
        [1-s, np.power(1-s, 2), np.power(1-s, 2), np.power(1-s, 2), np.power(1-s, 3)]
    )

    return -integrate.tplquad(
        fun3, 0, L,
        lambda x: x, lambda x: L, lambda x, y: y, lambda x, y: L,
        args=[gs-1, dur, L_matrix, D, v0])[0]*pow((1-1/2/N)*(1-2/2/N), gs-1) / L / L / L * 6

def getk4(s, gs, dur, L=1, N=1000):
    M = np.array(
        [
...
cdef fun3(double l, double ls, double lss,
    ...
    cdef double lls = (1 + np.exp(-2*d1)) / 2
    cdef double ldls = (1 - np.exp(-2*d1)) / 2
    cdef double lsdlss = (1 - np.exp(-2*d3)) / 2
    cdef double lslss = (1 + np.exp(-2*d3)) / 2
#     THE SAME - ну и треш
...
#         double gs, double dur, M,
#         double[:, ::1] L, D, v0):
#
#     cdef double d1 = abs(b - a)
#     cdef double d2 = abs(c - b)
#     cdef double d3 = abs(d - c)
#
#     # Haldane map function
#     cdef double ab = (1 + np.exp(-2*d1)) / 2
#     cdef double bc = (1 + np.exp(-2*d2)) / 2
#     cdef double cd = (1 + np.exp(-2*d3)) / 2
#
#     cdef double ac = (1 + np.exp(-2*(d1+d2))) / 2
#     cdef double bd = (1 + np.exp(-2*(d2+d3))) / 2
#
#     R = np.array(
#         [
#             [ab*bc*cd],
...
#     )
#
#     U, Q = np.linalg.eig(M@R)
#     U = np.diag(U**(gs))
#     Q_inv = np.linalg.solve(Q, np.eye(4))
#
#     w = (eigv @ Q) @ U @ (Q_inv @ w)
#     return w[0]
